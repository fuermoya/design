	...
	return func(c *gin.Context) {
		...
		if claims.ExpiresAt.Unix()-time.Now().Unix() < claims.BufferTime {
			...
			if global.CONFIG.System.UseMultipoint {
				// 获取用户当前的JWT token
				RedisJwtToken, ok := jwtService.GetCacheJWT(newClaims.Username)
				if ok && RedisJwtToken != token {
					// 如果存在不同的token，将旧token加入黑名单
					_ = jwtService.JsonInBlacklist(system.JwtBlacklist{Jwt: RedisJwtToken})
				}
				// 更新缓存中的JWT token
				_ = jwtService.SetCacheJWT(newToken, newClaims.Username)
			}
		}
	}
}
